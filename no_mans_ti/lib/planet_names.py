"""
Shamelessly 'inspired' by: https://github.com/sayamqazi/planet-name-generator
"""
import random
...
def build_syllables_list():
    ...
    for p in PLANETS:
        ...
        for lx in lex:
            if lx not in syllables:
                syllables.append(lx)

    return syllables, total_syllables


SYLLABLES, TOTAL_SYLLABLES = build_syllables_list()
...
def random_planet_name():
    ...
    while length > 0:
        while 1 not in FREQUENCY[initial]:
            initial = random.randint(0, size - 2)
        planet_name += SYLLABLES[initial]
        initial = FREQUENCY[initial].index(1)
        length -= 1
